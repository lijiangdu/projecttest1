...
# args
def mslic(filename,imgfile):
    # if not os.path.exists(imgfile):
    #     # 新建文件夹
    #     os.makedirs(imgfile)
    #filename = "E:\\project\\data\\t2\\TrainingImg1\\train_000_0000\\68.png"

    # load the image and apply SLIC and extract (approximately)
    # the supplied number of segments
    image = cv2.imread(filename)
    # segments = slic(img_as_float(image), n_segments=100, sigma=5)
    slic = cv2.ximgproc.createSuperpixelSLIC(image, cv2.ximgproc.MSLIC, region_size=10, ruler=40.0)
